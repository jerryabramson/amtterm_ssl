...
static char *amt_clientcert;
static char *amt_clientkey;
...
static int gamt_connect(struct gamt_window *gamt)
{
    ...
    gamt->redir.clientcert   = amt_clientcert;
    gamt->redir.clientkey   = amt_clientkey;
...
main(int argc, char *argv[])
{
    ...
    for (;;) {
        if (-1 == (c = getopt(argc, argv, "hdtu:p:f:c:b:l:LC:c:k:")))
            break;
        switch (c) {
            case 'd':
                amt_debug++;
            ...
            case 'C':
                ...
                break;
            case 'c':
                amt_clientcert = optarg;
                break;
            case 'k':
                amt_clientkey = optarg;
                break;
#endif
            case 'f':
